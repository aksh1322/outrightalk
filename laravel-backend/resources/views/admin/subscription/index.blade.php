    ...
    <table class="table table-condensed">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $val->title }}</td>
                <td>{{ $val->type }}</td>
                <td>{{ $val->color_title }}</td>
                <td>{{ $val->color_code }}</td>
                <td>{{$val->type =="room" ? ($val->room_capacity == 0 ? 'Unlimited' :$val->room_capacity  ): "n/a" }}</td>
                <td>{{$val->type =="room" ?$val->ban_limit : "n/a"}}</td>
                <td width="10%">
                    @if($val->icon['thumb'])
                ...
                <td class="text-center">
                    @if($permission['edit'])
                        <a href="{{ route($routePrefix . '.priceIndex',$val->id) }}" class="btn btn-outline-success waves-effect" title="Price" data-original-title="Price">{!! \Config::get('settings.icon_sort_asc') !!}</a>
