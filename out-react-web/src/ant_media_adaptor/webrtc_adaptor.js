 ...
 */
 ...
 export default function WebRTCAdaptor(initialValues)
 {
	 ...
	 thiz.localVideo = document.querySelector(thiz.localVideoId);
	 thiz.remoteVideo = document.querySelector(thiz.remoteVideoId);
	 ...
	 this.publish = function (streamId, token, stream) {
		thiz.localStream = thiz.localStream ? thiz.localStream : stream
		var jsCmd = {
			command : "publish",
			streamId : streamId,
			token : token,
	 ...
	 function WebSocketAdaptor() {
		 ...
		 wsConn.onopen = function() {
			 ...
			 connected = true;
			 thiz.callback("initialized", thiz);
		 }
