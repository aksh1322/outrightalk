...
function VideoMessageList() {
    const videoList = useAppMessageList()
    ...
    const onCheckSelectAll = (evt: any) => {
        ...
        if (evt) {
            if (videoList && videoList.unread_message.length) {
                for (let k = 0; k < videoList.unread_message.length; k++) {

                    if (videoList.unread_message[k].deleted_at == null) {
                        let exist = tempCheckedValues.indexOf(videoList.unread_message[k].id)
                        if (exist == -1) {
                            tempCheckedValues.push(videoList.unread_message[k].id)
                        }
                    }
            ...
            if (videoList && videoList.old_message.length) {
                for (let k = 0; k < videoList.old_message.length; k++) {

                    if (videoList.old_message[k].deleted_at == null) {
                        let exist = tempCheckedValues.indexOf(videoList.old_message[k].id)
                        if (exist == -1) {
                            tempCheckedValues.push(videoList.old_message[k].id)
                        }
                    }
    ...
    const showAlert = (e: React.MouseEvent, ids: number[]) => {
        ...
        setAlert(
            <SweetAlert
                ...
                title={`Delete ${ids.length > 1 ? 'Video Messages' : 'Video Message'}`}
                onConfirm={() => deleteVideoMessage({ voicemail_id: ids.map(x => x) })}
                onCancel={hideAlert}
                focusCancelBtn={true}
            >
                {`Are you sure to delete ${ids.length > 1 ? 'videomessages' : 'videomessage'}?`}
            </SweetAlert>
        );
    }
    ...
    const deleteVideoMessage = (params: any) => {
    ...
    const handleViewMessage = (e: React.MouseEvent, id: number, viewStatus: number) => {
        ...
        if (userSelector?.is_subscribed) {
            const limit: number = getVoiceVideoMessageStorageLimit(userSelector && userSelector.is_subscribed && userSelector.is_subscribed.feature, 'video')
            if (videoList && videoList.old_message.length < limit) {
                viewVideoMessage({ record_id: id })
            } else {
                toast.error(CUSTOM_MESSAGE.OTHERS.VIDEO_OLD_MSG_LIMIT)
            }
        } else {
            if (videoList && videoList.old_message.length < 2) {
                viewVideoMessage({ record_id: id })
            } else {
                toast.error(CUSTOM_MESSAGE.OTHERS.VIDEO_OLD_MSG_LIMIT)
            }
    ...
    const isShowMasterCheckBox = () => {
        if ((videoList && videoList.unread_message.length) || (videoList && videoList.old_message.length)) {
            var unreadFound = videoList && videoList.unread_message.length ? videoList.unread_message.filter((msg: any) => msg.deleted_at == null) : [];
            var oldFound = videoList && videoList.old_message.length ? videoList.old_message.filter((msg: any) => msg.deleted_at == null) : [];
        ...
        <React.Fragment>
            <div className="table-responsive mb-0" data-pattern="priority-columns">
                ...
                <table className="table">
                            ...
                            <th style={{ width: 80 }}>
                                {isShowMasterCheckBox() ?
                                    <div className="custom-control custom-checkbox custom-checkbox-outline theme-custom-checkbox custom-checkbox-success">
                                        <input type="checkbox" className="custom-control-input" id="master-select-video"
                                            checked={selectMaster()}
                                            onChange={(evt) => {
                                                onCheckSelectAll(evt.target.checked)
                                            }} />
                                        <label className="custom-control-label" htmlFor="master-select-video" />
                                    </div> : null}
                            </th>
                    ...
                    <tbody>
                        {videoList && videoList.unread_message.length ? videoList.unread_message.map((x: any, index: number) =>
                            <tr className={
                                clsx({
                    ...
                    <tbody className="tbody-no-border">
                        ...
                        {
                            videoList && videoList.old_message.length ? videoList.old_message.map((x: any, index: number) =>
                                <tr className={
                                    clsx({
                                        'deleted': x.deleted_at != null,
                                        'unread': x.deleted_at === null
                    ...
                    {
                        videoList && videoList.deleted_message.length ? videoList.deleted_message.map((x: any, index: number) =>
