...
import {
  useAppVideoVoiceMessageShareIds,
  useAppVideoVoiceMessageShareDateTime,
  useAppReplyMessageSelector,
} from "src/_common/hooks/selectors/videoMessageSelector";
...
import { useAppLoader } from "../../../../_common/hooks/actions/common/appLoaderHook";
...
export default function SendVideoMessagePmModal({
  shouldShow,
  onClose,
}: SendVideoMessageModalProps) {
  ...
  const [videoDeviceAvailable, setVideoDeviceAvailable] =
  ...
  const {
    ...
    handleSubmit,
    errors,
  } = useForm<sendVideoMessageFormValues>({
    // mode: 'onBlur',
  ...
  const onSubmit = (values: sendVideoMessageFormValues) => {
    ...
    if (pmOtherUsers && pmOtherUsers.length) {
      let pmUsers: number[] = pmOtherUsers.map((x: any) => x.user_id);
      let fd = new FormData();
      ...
      videoVoiceMessageApi.callSendMessage(
        fd,
      ...
      toast.error("No users available for send video message");
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      >
        <Modal.Header>
          <h5 className="modal-title mt-0">Send Video Message</h5>
          <button
            type="button"
            className="close"
            ...
            <div className="manage-video-message-panel">
              <form onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                <div className="message-modal-btns d-flex justify-content-between">
                  {mediaBlob && status == "stopped" ? (
                    <a
                      href="#"
                      className="btn theme-btn btn-danger waves-effect"
                      onClick={(e) => handleResetRecord(e)}
                    >
                      Reset
                    </a>
                  ) : null}
                  <div className="d-flex">
                    <a
                      ...
                      className="btn theme-btn btn-default mr-2 waves-effect"
                    >
                      Cancel
                    </a>
                    <button
