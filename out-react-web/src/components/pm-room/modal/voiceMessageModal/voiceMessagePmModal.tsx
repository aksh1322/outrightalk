...
export default function SendVoiceMessagePmModal({ shouldShow, onClose }: SendVoiceMailModalProps) {
    ...
    const onSubmit = (values: sendVoiceMailFormValues) => {
        ...
        let pmOtherUsers = pmWindowDetailsSelector && pmWindowDetailsSelector.users && pmWindowDetailsSelector.users.length ?
            pmWindowDetailsSelector.users.filter((x: any) => x.user_id != userSelector?.id) : []
        if (pmOtherUsers && pmOtherUsers.length) {
            ...
            toast.error("No users available for send Voice message")
        }
    }
